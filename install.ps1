...
function Downgrade-OpusToFallback {
    foreach ($agent in @("forge-planner.md", "forge-discusser.md", "forge-researcher.md")) {
        ...
        if ($DryRun) {
            ...
            $content = $content -replace '(?m)^model: "claude-opus-4-8\[1m\]"$', 'model: "claude-opus-4-7[1m]"'
...
# Replaces both `claude-opus-4-7[1m]` and `claude-opus-4-8[1m]` with $Target.
# Touches only opus model strings — sonnet/haiku and user customizations for other
# models are preserved. If user explicitly pinned a phase to claude-opus-4-7[1m], they
...
$script:OpusTarget = "claude-opus-4-8[1m]"  # default; flipped to claude-opus-4-7[1m] on downgrade
